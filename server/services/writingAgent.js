...
async function reviewChapter(chapterContent, novel, lastChapters) {
  ...
  const prompt = `请审查以下小说章节内容，检查是否存在以下问题（朱雀AI检测专项版）：

1. 【衔接问题】是否与上一章结尾顺畅衔接（上一章结尾：${lastEnding ? lastEnding.substring(0, 100) : '无'}）
2. 【重复问题】是否有与前面章节重复的段落或描述
3. 【角色问题】角色性格和行为是否与之前一致（主角：${novel.protagonistName || '未知'}）；不同角色对话是否有区分度（不会所有角色说话方式都一模一样）
4. 【节奏问题】是否有大段无意义的描述或对话拖沓；段落长度是否有变化（避免连续3段同等长度）
5. 【AI味问题 — 朱雀专项】是否有以下朱雀高频标记词：
   - 连接词类：值得注意的是、不可否认、综上所述、与此同时、总而言之、众所周知、进一步来说、从这个角度来看、基于以上分析
   - 假深度句：他终于明白、她这才意识到、这一刻终于、生活就是这样
   - 修饰词类：仿佛、好像、不禁、微微、一丝、一抹、眼中闪过、嘴角勾起、心中一动
6. 【机械标签】对话是否使用"XX说道""XX淡淡地说"等机械标签（应改为动作描写）
7. 【具体细节】是否有具体的时间/地点/数字/感官细节（气味、声音、触感、温度），还是抽象描述堆砌
